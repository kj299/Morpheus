...
| **First detections** | Four deterministic layer 2 rules as saved searches in the app: a MAC in two places at once, 802.1X authorization with no authentication in front of it, more MACs than permitted on a single-host port, and an address claimed by more than one MAC. The last two depend on a list the estate owns and ship with the hook for it, firing on nothing until it is populated. All four predicates asserted in Python over the planted corpus. Not yet run on a live search head |

Fourteen stages and eighteen supporting modules, covered by 855 tests.
