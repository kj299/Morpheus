...
| **TC-0 context store** | Identity and asset facts held bitemporally: each version carries the interval it was true for and the instant the source recorded it, so the store answers what held at a time both as it was known then and as it is known now. A correction is a new version rather than an edit, a retraction ends a fact for exactly the interval it names, and a full daily snapshot is diffed against what is already known so only real changes are recorded -- an absence becoming a retraction from the snapshot onward. Profiles, one fact per group membership, and asset owner, criticality, data classification and peer group, the peer group read from the inventory rather than computed. A point-in-time join attaches the context to any event, by default as known at the event's own time, so a correction recorded next week cannot change what an enrichment of today's events said; a record without the instant its source recorded it is refused rather than stamped with the moment it arrived. The composed pipeline runs under control 13's six checks over a corpus built from the cases where the two views disagree -- a late leaver, a retroactive correction, a late reclassification -- beside a move recorded on time where they must not. No rules of its own; the layer 7 SaaS and endpoint rules read it |
| **Layer 1 (TC-1)** | Interface counter normalization that tells a counter wrap from a device reboot, transceiver and neighbor novelty, optical power scored against each port's own rolling baseline, link flap counting that catches flaps between two polls, identifier change detection with no period boundary, and port bindings closed into the resolvable intervals the `binding_l1` lookup is built from, which completes the identifier ladder: a chain query now reaches a site, an optic and an LLDP neighbour rather than stopping at a port name. The lookup answers in two tenses: one unbucketed row per port for what is in it now, and beside it a history holding only the intervals that row has overwritten, bucketed at a day, so an investigation into last Tuesday resolves to last Tuesday's optic instead of to the one installed on Wednesday. A port nobody has touched appears in the history not at all, which is what makes it affordable |
...
| **Layer 3 (TC-3)** | Fan-out, fan-in and distinct destination ports per source over a trailing window; destination classification through the shared `parsers/ip.py` rather than a private copy of the rules, with the internal-destination proportion that separates a scan from a browser reaching just as many places on the internet; byte asymmetry and permanent per-source novelty of the destination network; the coefficient of variation of a directed pair's inter-arrival times and payload sizes, which is what beaconing consists of and is scale-free so one threshold covers a beacon every minute and one every hour; and each flow's IP time-to-live against its source's own established value, where one hop is what an interposed device costs. Five detections ship as saved searches, each asserted over a seeded corpus together with the case beside it that must stay quiet -- a scan against a browser, a beacon against a person, a tapped host against a steady one. The composed pipeline runs under control 13's six checks |
| **Layer 4 (TC-4)** | The thirteen transport features the shipped `abp-pcap-xgb` model was trained on, rolled up per flow per time bin, and a transfer envelope kept per `(src_ip, dst_ip, dst_port)` triple against that triple's own 99th percentile rather than a global threshold. Three departures from the reference preprocessing are corrections rather than preferences: the rollup is a running total instead of a group within whatever message the stream happened to be chunked into, which is the defect determinism control 5 exists to catch; the bin is floored and half-open like every other window in this fork rather than labelled by its end; and the envelope is a percentile only where a hundred prior transfers support one, because below that a 99th percentile by nearest rank is the largest transfer ever seen wearing a percentile's name. Three of the five detections ship as saved searches -- the other two need a Triton endpoint and a layer 7 this fork does not have, and say so -- each asserted over a seeded corpus with the case beside it that must stay quiet: a port scan against a workstation whose handshakes complete, a closed-port sweep against a service outage that produces the identical ratio pointed the other way, and an exfiltration against a busy host that moves more data in total. The composed pipeline runs under control 13's six checks |
| **Layer 6 (TC-6)** | Client fingerprint novelty per host, certificate issuer novelty per destination, self-signed certificates on connections that left the estate, cipher downgrade against each pair's own historical floor, and declared-versus-detected content type crossing a category boundary. The issuer and cipher rules decline until their entity has a history; the self-signed and content rules answer on a destination's first handshake; and the fingerprint rule answers immediately but is gated on the host having a settled one. The harness asserts each of those separately, because "the rule is quiet" means something different in each case. Two judgement tables ship with the rules the guide says they must: an explicit six-tier cipher strength ordering and a media type category map, both of which report what they could not place rather than guessing, and a coverage search that makes that gap visible rather than leaving it as a rule that quietly never fires. Five detections ship as saved searches, each asserted over a seeded corpus with the case beside it that must stay quiet: a settled host acquiring a stack against one the estate has just started seeing, an interception against a delivery host legitimately behind four authorities, an external self-signed certificate against the internal appliance's ordinary one, a downgrade against a pair whose own suite varies, and a file behind a declared image against the re-encodings an estate produces in thousands. The composed pipeline runs under control 13's six checks |
| **Layer 7 (TC-7), DNS, HTTP, SaaS and endpoint** | Subdomain entropy, mean label length and distinct subdomains per registered domain for DNS, with the registered domain found by the full Public Suffix List algorithm -- wildcards, exceptions and the private section -- over the list already bundled with Morpheus. Entropy is measured below the registered domain, where ordinary names score under 2.3 bits per character, rather than over the whole name, where they score 3.0 to 3.7. For HTTP, 4xx and 2xx counts, their ratio and distinct paths per client over a trailing ten minutes. Both seal on the querying client. Two detections ship as saved searches: DNS tunnelling, which counts distinct subdomains among the queries that clear both other conditions rather than across all of a domain's traffic, and enumeration, evaluated as a multiplication so a client that never received a success still fires. Each of the rule's conditions has a benign case in the corpus that it alone keeps quiet -- a delivery network with random but short labels, a tenant domain with too few names, a crawler that mostly succeeds, a broken client hammering a few paths. For SaaS, each operation's record count against the principal's own 30-day 99th percentile for that operation, and the distinct object types reached per Monday-to-Monday week, after the TC-0 context store has attached the principal's group memberships and the object's data classification as known at the time. Two more detections ship on them: bulk data access, where the classification sets the severity rather than gating the alert, and access breadth rising for four weeks with the role unchanged, a watchlist like layer 5's drift rule. Their controls are a routine heavy exporter, one just under five times, a newcomer with no baseline, a denied export, a rise with a role change in it, and a rise too short -- and a role change recorded after the rise, which the rule watchlists because nothing said the role had changed. For endpoint, whether a process's parent and image are a pair neither its host nor its host's peer group has run in thirty days, with the peer group read from the TC-0 inventory as it stood when the process ran, paths case-folded and per-user profile folders collapsed, nothing judged until the history compared against spans a week, and processes that share a second judged against what came before that second rather than against each other. One more detection ships on it, process ancestry novelty, the integrity level setting its severity; its controls are a compile the host has never run and its peer runs daily, an editor installed under a different user's profile, a pair the host ran last week, and a machine four days old -- while a pair routine in another peer group, and one last seen thirty-eight days ago, fire. The model rule at this layer is not built; see the guide. All three composed pipelines run under control 13's six checks |
| **Chained rules** | All three that ship read the scored events rather than other rules' notables, so they fire on pipeline output alone; R-C-002, a new TLS fingerprint followed within the hour by beaconing to the same destination, first read notables and never returned a row until it was rewritten. R-C-001, lateral movement: within thirty minutes and in order, a source's fan-out rising above its own previous hour, a principal's first login from that source to a server, and a process on that server whose ancestry neither it nor its peer group has run in thirty days. No step has to breach a threshold of its own -- the reference campaign reaches half the fan-out the single-layer rule fires on. It reads the scored events rather than other rules' notables, so unlike R-C-002 it fires on pipeline output alone, and it joins its steps on values -- the flow's source to the login's, the login's target to the process's host -- because the three layers are sealed on three different entities and no lineage chain holds them all. Its declared join tolerance is two minutes, twice an assumed sixty-second clock offset. Layer 5 gained a target-host novelty feature for it. Asserted over a corpus of one estate seen by three collectors, with six actors each one step short: wrong order, too slow, a known server, another source, a flat fan-out, and no new process. R-C-004, staged exfiltration: within two hours and in order, a principal's bulk SaaS export, a transfer envelope breach from an address one of their sessions held open at that moment, and a connection from that address to a destination whose certificate issuer nobody in the estate has seen in thirty days -- a feature layer 6 gained for it, kept estate-wide and waiting a week before it answers. Its controls are a breach from another principal's address, a breach after logging off, the corporate issuer, and the export last rather than first; every one of their exports and breaches is a single-layer notable on its own |
| **Layer 5 (TC-5), deterministic half** | Session assembly from the separate start and stop records most identity providers emit, the digital fingerprinting features this layer is the sweet spot for (`logcount`, `locincrement`, `appincrement`, a device increment, distinct source ASNs per window), hour-of-day and day-of-week deviation scored against each principal's own histogram rather than a population's, implied travel speed between consecutive successful authentications, the failure and multi-factor denial runs that end in a success, and the drift trajectory R-P-L5-006 reads -- velocity, acceleration, the length of the rising run and its total rise in the principal's own standard deviations. The trajectory is measured over daily windows sealed behind the hourly ones, one observation per principal per day. The composed pipeline runs under control 13's six checks; the two deterministic rules and the predictive one ship as saved searches, and the predictive one fires on the reference corpus for reasons that are arithmetic rather than behaviour, each of them written down. The autoencoder can now occupy the same slot through `morpheus.utils.dfencoder_scorer`, and `run_model.py` runs the composed pipeline with it as its fourth check on the machine with the card. That check failed its first run, on the adapter letting the size of a message's row group reach the model, and passes now that the adapter asks for one row at a time: 105 rows scored identically twice over and again under the batch-split sweep, with the models pinned to a digest of their own weights -- see below |
...
| **Determinism** | A total row order imposed before any stateful stage, frame canonicalization and digesting, score quantization, and a CI harness running control 13's six checks against all twelve composed pipelines -- lineage, layer 1 and 2 telemetry, layer 3 flows, layer 4 packets, layer 5 sessions, layer 6 handshakes, layer 7 queries, requests, SaaS operations and process starts, the TC-0 context store and its point-in-time join, the estate that joins layers 1, 2 and 5 over one hour, and the campaign that tells two intrusions across layers 3 to 7 -- over seeded corpora with planted anomalies and the negative controls beside them |
...
| **Personal data** | An inventory of what the pipeline holds about a person, classified per column and checked against the four reference pipelines so a new feature column fails a test until somebody has decided what it says: eleven columns identify a person, eighteen address their device, fourteen locate them, eleven are pseudonyms, and a hundred and eighty are profile, a hundred and seventy-four of them behavioural -- the largest category is the one the design derives rather than the one it ingested. Beside it a minimization stage that drops or pseudonymizes at the wire boundary, with a keyed HMAC, no default key, stable digests so the per-entity story survives, and a refusal to pseudonymize any column whose domain its own definition bounds. Its limits are asserted rather than noted in passing: a layer 5 record with every name and the desk port digested is re-identified through its own chain one hop down, so this is pseudonymization and never anonymization |
| **First detections** | Seven rules as saved searches in the app, six deterministic and one predictive. Two deterministic at layer 5: a principal authenticated from two places faster than the journey can be made, and a run of multi-factor denials ended by an approval. The first excludes token refreshes and VPN egress ranges from the measurement *and* from becoming the location the next one is measured against, and one intrusion produces a pair of alerts rather than one. And four at layer 2: a MAC in two places at once, 802.1X authorization with no authentication in front of it, more MACs than permitted on a single-host port, and an address claimed by more than one MAC. The first fires on the interval between the two sightings rather than on their end reason, because an estate polls its switches in sequence and a cross-switch spoof is therefore seconds apart rather than simultaneous. The last two depend on a list the estate owns and ship with the hook for it. R-D-L2-001 fires on nothing until its port designation lookup is populated; R-D-L2-003 is the opposite, and fires on every redundancy gateway until its exclusion list is supplied. The predictive one, R-P-L5-006, reads the daily drift trajectory and fires on three principals in the reference corpus, none of them behaviour: two are frozen reference parameters meeting cumulative features, one is the planted burst ending a shallow run. Every predicate is asserted in Python over the planted corpus, the predictive one down to the exact principal-days. Not yet run on a live search head |

Forty-four stages and thirty-nine supporting modules, covered by 1,773 distinct tests.
...
- **A correlation chain now spans three layers, and the two ways it could have cheated are tested.** The
  estate pipeline runs layers 1, 2 and 5 over one hour of one estate and carries each principal down to
  the port they sat at: a supplied directory takes `user_principal` to `dot1x_identity`, and the 802.1X
  bindings take that to the port, whose key is layer 1's `entity_key` byte for byte. Fifteen chains hold
  all three layers. The remote worker the directory does not name reaches no port and shares no chain
  with one, and a sign-in made after the hour's last 802.1X exchange reaches no port either, because a
  binding is an interval rather than a lookup. Attribution that is confidently wrong would be worse than
  a chain that stopped at two layers, so both are asserted and the harness was sabotaged four ways to
  prove they bite.
...
- **The fourth check earned its place by failing.** Its first run, on 2026-09-13, was reproducible twice
  over and not reproducible under the batch-split sweep. The adapter was handing the model a frame whose
  shape was the size of whatever row group a message happened to carry, and a network is not
  shape-invariant to the last decimal; `drift_rise_sigmas` divides a rise by the spread of a few nearly
  equal scores, so a seventh-place difference in a loss arrived as tenths in the column a rule reads. The
  adapter now asks for one row at a time. Across the failed run and the passing one the five weight
  digests are identical and `pipeline_mean_abs_z_max` is 2.3823 in both, so training is reproducible
  across days and the repair changed how rows were handed over rather than what was fitted or what it
  scored.
- **What that run does not establish is that the model detects anything.** Five principals over one week
  ...
  living in a document beside them. The composed pipeline now produces `mean_abs_z` from `ReferenceScorer`,
  frozen arithmetic that its own docstring calls not a model, so R-B-L5-001 and R-P-L5-006 are evaluated
  against real columns: the first does not fire, the second fires on three principals for reasons that are
  arithmetic and are each written down. What is true is narrower than "predictive" and worth having: the
  path from features to scores to trajectory gives the same answer twice, and its first false positives are
  known.
...
| `chain_anchor`, `chain_anchor_source` | What the record's correlation chain is rooted on, and which column supplied it. A layer 2 observation the ladder resolved to a port roots on the port and shares the port's `lineage_id` across layers; `chain_anchor_source` says it got there through the binding table rather than by direct observation. A layer 5 authentication roots the same way once a directory and the 802.1X bindings have carried its principal down to a port, which is what makes a chain span three layers. |
...
| --- | --- | --- | --- |
| **TC-0** Identity and asset context | user, asset | HR system, IdP groups, CMDB, asset inventory | Three stages ship: an identity and an asset producer, and the point-in-time join that attaches context to an event. **Bitemporal** is a hard requirement: valid time *and* transaction time, or you cannot answer "was this user in Finance on March 3rd" |
...
| **TC-3** Network | `src_ip`, and the directed pair | NetFlow v9, IPFIX, sFlow, VPC and cloud flow logs, firewall session logs | Four stages ship, and five detections with them |
| **TC-4** Transport | `flow_id`, plus `community_id` for cross-tool joins | The TC-3 sources with per-packet detail, Zeek `conn.log` | Two stages ship, carrying the thirteen features the shipped `abp-pcap-xgb` model was trained on and a per-triple transfer envelope; `community_id` ships beside them and is verified against the reference vectors |
...
| **TC-6** Presentation | `ja4_client`, plus `certificate_fingerprint_sha256` | TLS inspection points, Zeek `ssl.log` and `x509.log`, load balancer logs, certificate transparency | Four stages ship, and five detections with them |
| **TC-7** Application | Varies: `user_principal` for SaaS, `hostname` for DNS and endpoint | HTTP proxies and WAFs, DNS resolvers, SaaS audit APIs, database audit logs, API gateways, EDR | Four stages ship, DNS, HTTP, SaaS and endpoint, and five detections with them; the sixth needs a Triton server |
...
**What is most sensitive here, in order.** The single most fragile predicate this repository shipped was
...
detection that looks missed -- the rule simply does not fire. Every chained rule now declares a join tolerance of
two minutes instead, twice an assumed sixty-second offset, and lets a step precede the one it follows by that much.
Below that:
...
The pipeline emits all fourteen sourcetypes the shipped app parses -- see
[`siem_sourcetypes`](./python/morpheus/morpheus/utils/siem_sourcetypes.py), which names each one's producer
and would say, for any stanza shipped ahead of its producer, exactly what would have to be built.
`TA-morpheus-lineage` is an installable Splunk app for the consuming side
([README](./examples/splunk_lineage_app/README.md)).
