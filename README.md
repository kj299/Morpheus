...
Twenty-three stages and twenty-seven supporting modules, covered by 1,834 tests.
...
| **TC-1** Physical | `site_id:device_id:port_id` | SNMP interface tables, optical transceiver diagnostics, LLDP/CDP neighbors, patch-panel inventory | Six stages ship, the newest closing port observations into the intervals `binding:l1` is built from; wiring it into the composed pipeline is what will finally populate that lookup |
