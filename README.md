...
| **Determinism** | A total row order imposed before any stateful stage, frame canonicalization and digesting, score quantization, and a CI harness running control 13's six checks against all nine composed pipelines -- lineage, layer 1 and 2 telemetry, layer 3 flows, layer 4 packets, layer 5 sessions, layer 6 handshakes, layer 7 queries and requests, the TC-0 context store and its point-in-time join, and the estate that joins layers 1, 2 and 5 over one hour -- over seeded corpora with planted anomalies and the negative controls beside them |
...
Forty-two stages and thirty-eight supporting modules, covered by 1,633 distinct tests.
...
| **TC-7** Application | Varies: `user_principal` for SaaS, `hostname` for DNS, `process_guid` for endpoint | HTTP proxies and WAFs, DNS resolvers, SaaS audit APIs, database audit logs, API gateways, EDR | Two stages ship, DNS and HTTP, and two detections with them; SaaS and endpoint wait on TC-0 |
