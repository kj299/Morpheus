...
- Determinism Stamp Stage {py:class}`~morpheus.stages.lineage.determinism_stamp_stage.DeterminismStampStage` Write the determinism envelope -- tier, pipeline fingerprint, configuration hash, code commit, image digest, feature schema version and seed -- onto every event, and the pinned model each entity was scored against onto every scored one. Two events with the same fingerprint were produced by the same system; two with different ones are not directly comparable, and a drift analysis that ignores the difference attributes a configuration change to a behavioral change. A model that was substituted for a missing one is marked, because an event scored against a population model is a different claim from one scored against the entity's own.
...
- Total Order Stage {py:class}`~morpheus.stages.lineage.total_order_stage.TotalOrderStage` Sort every batch by `event_time`, `collector_id` and `collector_seq` before the first stateful stage, which is determinism control 8 as a stage. The telemetry stages flag out-of-order arrival rather than repairing it, so something has to impose the order once; this does, and refuses ties rather than falling back on the input's own arrangement.
...
- TC-1 Change Stage {py:class}`~morpheus.stages.telemetry.tc1_change_stage.TC1ChangeStage` Report when a port's `transceiver_serial` or `lldp_neighbor_chassis_id` differs from the previous sample, and whether that value has ever been seen on the port before. Holds the previous value per port rather than bucketing by period, so a change is detected however far apart the samples fall; prefer this to the period-bucketed counts when the question is whether an identifier changed.
- TC-1 Feature Stage {py:class}`~morpheus.stages.telemetry.tc1_feature_stage.TC1FeatureStage` Count distinct `transceiver_serial` and `lldp_neighbor_chassis_id` values per port per period, so that hardware substitution and topology change read as a count above one against a steady state of one. Imposes a total row order before counting, because the underlying primitives are cumulative and an unsorted batch silently produces different features, and warns when a frame straddles a period boundary, where the count would reset and hide a change.
- TC-1 Flap Stage {py:class}`~morpheus.stages.telemetry.tc1_flap_stage.TC1FlapStage` Count link state transitions per port per interval, using the device's own record of when the interface last changed so that a port which drops and recovers inside one polling gap is counted rather than read as stable. Every count is a lower bound, and a device restart is counted but labelled.
...
- TC-1 Optical Stage {py:class}`~morpheus.stages.telemetry.tc1_optical_stage.TC1OpticalStage` Score each port's optical transmit and receive power against a rolling median of that port's own recent readings, so that degradation and the step loss an inline tap causes are visible without an absolute threshold that no two links could share. Ports with no optic carry nulls rather than a fabricated baseline.
- TC-2 ARP Stage {py:class}`~morpheus.stages.telemetry.tc2_arp_stage.TC2ArpStage` Score the share of a sender's ARP that is gratuitous, rather than the raw count, so a chatty host is not mistaken for a poisoning one, and count the distinct MAC addresses claiming each sender address. Rows whose sender is a known first-hop redundancy address are marked rather than dropped.
- TC-2 Auth Stage {py:class}`~morpheus.stages.telemetry.tc2_auth_stage.TC2AuthStage` Pair each 802.1X exchange's start with its outcome to time authorization per port, counting retries, and flag an outcome that arrived with no exchange in front of it, which is what a bypass looks like from the switch.
- TC-2 Binding Stage {py:class}`~morpheus.stages.telemetry.tc2_binding_stage.TC2BindingStage` Turn layer 2 binding observations into the closed, half-open intervals `BindingTable` resolves against, inferring the end that no upstream source supplies and recording how it was arrived at. Each binding carries `port_key` as `site_id:switch_id:port_id`, identical to the layer 1 `entity_key`, so a resolved MAC lands on the string the TC-1 stages key on. Unlike the TC-1 stages this is not a pass-through: it emits one row per closed binding, so a batch in which nothing moved emits nothing. With `emit_open_bindings` it also emits one provisional record the moment a binding opens, with a null end for the consumer to cap, so live attribution has an answer inside the idle window.
- TC-2 Cardinality Stage {py:class}`~morpheus.stages.telemetry.tc2_cardinality_stage.TC2CardinalityStage` Count distinct MACs per port, ports per MAC, and OUIs per VLAN over a trailing window, which catch an unauthorized hub, a spoofed or moving device, and an unmanaged device class on a segment respectively. Each count carries a first-in-window flag and a saturation flag, the latter marking the count as a lower bound when a MAC flood pushes an entity past its sample cap.
- TC-5 Cadence Stage {py:class}`~morpheus.stages.telemetry.tc5_cadence_stage.TC5CadenceStage` Score an authentication's hour of day and day of week against the histogram of that principal's own history, smoothing an hour never used before into a finite surprise that grows with how much history it contradicts. The local time is a fixed offset rather than a named zone, so the feature cannot move when a timezone database is updated.
- TC-5 Risk Stage {py:class}`~morpheus.stages.telemetry.tc5_risk_stage.TC5RiskStage` Count a principal's authentication failures and multi-factor denials over a trailing window, the unbroken run immediately before each attempt, and the success that ends one, alongside the multi-factor to total authentication proportion over a wider window. The trailing success is what separates the pattern from the ordinary mistyped password, and it is what R-D-L5-004 fires on.
- TC-5 Travel Stage {py:class}`~morpheus.stages.telemetry.tc5_travel_stage.TC5TravelStage` Measure the speed a principal would have had to travel between consecutive successful authentications, which is what R-D-L5-003 reads. Token refreshes and known VPN egress ranges are excluded from the measurement and from becoming the location the next one is measured against, and every row records which of the two it was or why else it carries no measurement.
- TC-5 Novelty Stage {py:class}`~morpheus.stages.telemetry.tc5_novelty_stage.TC5NoveltyStage` Write the digital fingerprinting volume and novelty features for each principal -- `logcount` over a trailing window, and the cumulative `locincrement`, `appincrement` and device increments -- plus a windowed count of distinct source ASNs. A field the collector did not report is an omission rather than a value the principal used, so it does not raise an increment.
- TC-5 Session Stage {py:class}`~morpheus.stages.telemetry.tc5_session_stage.TC5SessionStage` Pair a session's start record with its separate end record to write how long it ran, for the identity providers and concentrators that emit the two apart and neither with a duration. Reports a stop with no start in front of it, and a session identifier that collected more than one start, rather than silently producing a null duration.
