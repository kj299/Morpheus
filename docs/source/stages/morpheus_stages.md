...
- TC-2 Binding Stage {py:class}`~morpheus.stages.telemetry.tc2_binding_stage.TC2BindingStage` Turn layer 2 binding observations into the closed, half-open intervals `BindingTable` resolves against, inferring the end that no upstream source supplies and recording how it was arrived at. Unlike the TC-1 stages this is not a pass-through: it emits one row per closed binding, so a batch in which nothing moved emits nothing.
