...
"""
Standalone driver for the TC-0 context pipeline's cross-restart check, and for regenerating its golden.

Runs the two TC-0 producers and the point-in-time enrichment in a fresh interpreter and writes the canonicalized
...
`golden_context_expected.csv` with it and reviews the diff.
"""
...
def main() -> int:
    ...
    result = context_pipeline.run_pipeline(config, context_pipeline.build_corpus())

    with open(sys.argv[1], "w", encoding="utf-8", newline="") as handle:
        handle.write(context_pipeline.render(result))
