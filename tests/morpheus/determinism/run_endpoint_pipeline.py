...
"""
Standalone driver for the layer 7 endpoint pipeline's cross-restart check, and for regenerating its golden.

Runs the endpoint class, with its peer-group enrichment, in a fresh interpreter and writes the canonicalized
...
`golden_endpoint_expected.csv` with it and reviews the diff.
"""
...
def main() -> int:
    ...
    result = endpoint_pipeline.run_pipeline(config, endpoint_pipeline.build_corpus())

    with open(sys.argv[1], "w", encoding="utf-8", newline="") as handle:
        handle.write(endpoint_pipeline.render(result))
