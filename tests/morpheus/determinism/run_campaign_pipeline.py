...
"""
Standalone driver for the campaign corpus's cross-restart check, and for regenerating its golden.

Runs the campaign corpus through its three layers' pipelines in a fresh interpreter and writes the canonicalized
...
`golden_campaign_expected.csv` with it and reviews the diff.
"""
...
def main() -> int:
    ...
    result = campaign_pipeline.run_pipeline(config, campaign_pipeline.build_corpus())

    with open(sys.argv[1], "w", encoding="utf-8", newline="") as handle:
        handle.write(campaign_pipeline.render(result))
