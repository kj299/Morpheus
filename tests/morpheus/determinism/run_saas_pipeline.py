...
"""
Standalone driver for the layer 7 SaaS pipeline's cross-restart check, and for regenerating its golden.

Runs the SaaS class, with its enrichment and weekly trajectory, in a fresh interpreter and writes the canonicalized
...
`golden_saas_expected.csv` with it and reviews the diff.
"""
...
def main() -> int:
    ...
    result = saas_pipeline.run_pipeline(config, saas_pipeline.build_corpus())

    with open(sys.argv[1], "w", encoding="utf-8", newline="") as handle:
        handle.write(saas_pipeline.render(result))
